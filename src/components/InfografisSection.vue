<template>
  <section class="bg-gray-50/50 py-16 overflow-hidden">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
      ...
      <div class="absolute top-10 left-10 w-72 h-72 bg-green-100 rounded-full mix-blend-multiply filter blur-3xl opacity-30 pointer-events-none"></div>
      ...
      <div class="flex flex-col sm:flex-row sm:items-end justify-between mb-10 gap-4 relative z-10">
        <div>
          <div class="flex items-center gap-2 mb-2">
            ...
            <span class="text-green-600 font-bold uppercase tracking-wider text-sm">Visual Data</span>
          </div>
        ...
        <router-link
          ...
          class="group flex items-center gap-2 text-sm font-bold text-gray-600 hover:text-green-600 transition-colors"
        >
          Semua Infografis
      ...
      <div v-else class="relative group/carousel">
        ...
        >
          <div
            ...
            :key="item.id"
            class="snap-center w-[280px] sm:w-[320px] bg-white border border-gray-100 rounded-3xl shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] hover:shadow-xl hover:shadow-green-500/10 hover:border-green-200 transition-all duration-500 flex-shrink-0 group overflow-hidden cursor-pointer flex flex-col"
          ...
          >
            ...
            <div class="px-6 py-5 bg-white relative z-10 flex-1 flex flex-col justify-center">
              <span class="text-[10px] font-bold text-green-500 uppercase tracking-widest mb-1.5">{{ item.kategori || 'Infografis' }}</span>
              <h3 class="font-bold text-gray-900 text-lg leading-snug truncate group-hover:text-green-600 transition-colors" :title="item.judul">
                {{ item.judul }}
              </h3>
            </div>
          </div>
